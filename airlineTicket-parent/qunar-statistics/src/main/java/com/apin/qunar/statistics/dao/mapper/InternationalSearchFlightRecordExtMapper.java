...
public interface InternationalSearchFlightRecordExtMapper {
    @Select("select distinct account from international_search_flight_record where insert_time>#{startTime} and insert_time<=#{endTime}")
    List<String> queryAccountByInsertTime(@Param("startTime") Date startTime, @Param("endTime") Date endTime);

    @Select("select count(1) from international_search_flight_record where account=#{account} and insert_time>#{startTime} and insert_time<=#{endTime}")
